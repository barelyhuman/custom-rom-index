...
const getDevices = async ({
  ...
  if (searchTerm.length >= 3) {
    paginatedBaseQuery.andWhereRaw(
  ...
  let countQuery = db('roms_devices_mapping').count('roms_devices_mapping.id')

  if (status !== 'all')
    countQuery.andWhere({ 'roms_devices_mapping.status': status })
...
export { getDevices, upsertDevice }
